...
{
    m_dropperSolenoid1 = new frc::Solenoid(c_compressorPCMid, c_dropperSolenoidChannel1);
    m_dropperSolenoid2 = new frc::Solenoid(c_compressorPCMid, c_dropperSolenoidChannel2);
    m_limitSwitch1 = new frc::DigitalInput(c_dropperSensor1);
    m_limitSwitch2 = new frc::DigitalInput(c_dropperSensor2);
...
{
    frc::SmartDashboard::PutBoolean("Gear Drop Pin Sensed", IsPinSensed());
    if (m_position == kDown) {
        frc::SmartDashboard::PutString("Gear Drop Position", "Down");
    } else {
        frc::SmartDashboard::PutString("Gear Drop Position", "Up");
    }
    if (m_mode == kManual) {
        frc::SmartDashboard::PutString("Gear Drop Mode", "Manual");
    } else {
        frc::SmartDashboard::PutString("Gear Drop Mode", "Automatic");
    }
    frc::SmartDashboard::PutBoolean("Gear Drop Auto Triggered",m_autoTriggered);
    frc::SmartDashboard::PutNumber("Gear Drop Timer", m_timer->Get());
}
